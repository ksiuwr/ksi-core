...
export const discordRank = (app: Elysia) => app
    .derive(async ({ user }) => {
...
export const rankRequired = (rankRequired?: "DEV" | "ADMIN") => (app: Elysia) => app
    .use(discordRank)
    .derive(async ({ discordRank }) => {
        if (rankRequired === undefined && discordRank <= 1) throw status(403);
        if ((rankRequired === 'DEV' && discordRank !== DiscordRankStatus.RANK_DEV) || (rankRequired === 'ADMIN' && discordRank !== DiscordRankStatus.RANK_ADMIN)) throw status(403);
        return { discordRank }
    })